...
RSpec.describe 'get_drone_builds_for_github_branches' do
  ...
  #   Given the get request to drone will return some mocked up data
    ...
    stub_request(:get, 'https://drone.digital.homeoffice.gov.uk/api/repos/' + repo1_name + '/builds').
        to_return(:status => 200, :body => DRONE_DATA_REPO1, :headers => {:content_type => 'application/json'})
    stub_request(:get, 'https://drone.digital.homeoffice.gov.uk/api/repos/' + repo2_name + '/builds').
  ...
  #   And the get request to Github will return some mocked up data
    stub_request(:get, 'https://api.github.com/repos/' + repo1_name + '/branches').
        to_return(:status => 200, :body => GITHUB_BRANCHES1, :headers => {:content_type => 'application/json'})
    stub_request(:get, 'https://api.github.com/repos/' + repo2_name + '/branches').
...
RSpec.describe 'get_drone_builds' do
  it 'Gets all drone builds in the correct format for a single repo' do
    ...
    stub_request(:get, 'https://drone.digital.homeoffice.gov.uk/api/repos/' + repo_name + '/builds').
